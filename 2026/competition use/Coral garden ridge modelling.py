...
# Global variable for selected markers (vertices + custom midpoint markers)
# Default shows 8 cubes when the model starts.
selected_markers = ['A', 'D', 'BJ_MID', 'Q', 'R', 'M', 'N', 'CK_MID']
...
def get_marker_data(points):
    """Return selectable marker data: coordinate, cuboid id, and draw mode."""
    marker_data = {}
    for vertex, coord in points.items():
        marker_data[vertex] = {
            'coord': coord,
            'cuboid': {
                'A': 1, 'B': 2, 'C': 2, 'D': 1,
    ...
    kx, ky, kz = points['K']
    marker_data['BJ_MID'] = {
        'coord': ((bx + jx) / 2, (by + jy) / 2, (bz + jz) / 2),
        'cuboid': 2,
        'mode': 'center'
    }
    marker_data['CK_MID'] = {
        'coord': ((cx + kx) / 2, (cy + ky) / 2, (cz + kz) / 2),
        'cuboid': 2,
        'mode': 'center'
    }
    return marker_data
...
def plot_cuboids(points):
    ...
    marker_data = get_marker_data(points)

    # Draw cubes for all selected markers (all red now)
    for marker in selected_markers:
        if marker not in marker_data:
            continue
        vx, vy, vz = marker_data[marker]['coord']
        cuboid = marker_data[marker]['cuboid']
        mode = marker_data[marker]['mode']
        if cuboid == 1:
...
def submit_vertices(text):
    ...
    if not text:
        ...
        for p in parts:
            ...
            if normalized in marker_data:
                valid.append(normalized)
            else:
                print(f"Invalid marker: {p}. Ignored.")
        selected_markers = valid
    plot_cuboids(points)
