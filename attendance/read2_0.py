...
def _aio_publish_mqtt(value):
    ...
    if MQTTClient is None:
        ...
        return False
    if not _wifi_connect():
        print("Wi-Fi not connected; MQTT publish aborted")
        return False
    try:
...
def _aio_publish_resilient(value):
    ...
    if _aio_publish_rest(value):
        return True
    return _aio_publish_mqtt(value)
                            ...
                            if auth_ok:
                                ...
                                if blk_bytes is not None:
                                    ...
                                    if len(_normalize(candidate)) > 0:
                                        ...
                                        _aio_publish_resilient(candidate)
                                        sleep_ms(500)
                                    else:
